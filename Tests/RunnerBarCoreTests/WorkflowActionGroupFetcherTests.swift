...
struct WorkflowActionGroupFetcherTests {
    ...
    /// and failing the count assertion. The `callCount` assertion makes the
    /// regression signal explicit: exactly 3 transport calls for the three
    /// status endpoints, zero calls for `/jobs`.
    @Test func fetchActionGroups_concludedCacheEntry_jobsNotRefetched() async {
        ...
        // Exactly 3 API calls: in_progress, queued, completed — no /jobs.
