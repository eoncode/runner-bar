...
///
/// `@MainActor` is on `reload()` only — NOT on the class — so that `AppDelegate`
/// can hold this as a `lazy var` without a class-level actor requirement.
/// All callers of `reload()` already hop to main before calling it.
///
...
final class RunnerStoreObservable: ObservableObject {
    ...
    /// Must be called on the main thread — enforced by @MainActor.
    @MainActor
    func reload() {
