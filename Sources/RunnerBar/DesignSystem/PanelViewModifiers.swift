...
extension View {
    /// Applies `CardRowModifier` — flat semi-transparent surface for scrollable rows.
    ...
    /// Applies Liquid Glass card styling to a non-scrollable container (e.g. action rows, runner cards).
    /// macOS 26+: `.glassEffect(.regular, in: RoundedRectangle(cornerRadius:))`
    /// macOS < 26: `.background(rbSurfaceElevated)` + `.clipShape(RoundedRectangle)`
    ///
...
/// Backing modifier for `View.glassCard(cornerRadius:)`.
