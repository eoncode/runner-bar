...
// These replace large 3-member tuples in RunnerStore to satisfy `large_tuple`.
...
extension RunnerStore {
    ...
    func buildJobState(snapPrev: [Int: ActiveJob], snapCache: [Int: ActiveJob]) -> JobPollResult {
        ...
        log(
            "RunnerStore › \(liveJobs.filter { $0.status == "in_progress" }.count) in_progress "
            + "\(liveJobs.filter { $0.status == "queued" }.count) queued "
...
extension RunnerStore {
    ...
    func buildGroupState(
        ...
        freezeVanishedGroups(
            snapPrev: snapPrevGroups, liveIDs: liveIDs,
            now: now, into: &newCache
        )
        for group in doneGroups {
            var dimmed = group; dimmed.isDimmed = true
        ...
        log(
            "RunnerStore › groups: "
            + "\(liveGroups.filter { $0.groupStatus == .inProgress }.count) in_progress "
            + "\(liveGroups.filter { $0.groupStatus == .queued }.count) queued "
            + "| cache: \(newCache.count) | display: \(display.count)"
        )
    ...
    /// Rebuilds the cache keyed by head_sha for pass-through to `fetchActionGroups`.
    ...
    /// Removes cache entries whose head_sha appears in the freshly-fetched groups.
    ...
    /// Freezes groups that were live last poll but are absent this poll.
    private func freezeVanishedGroups(
        ...
        for (sha, group) in snapPrev where !liveIDs.contains(sha) {
            ...
            var frozen = group; frozen.isDimmed = true
    ...
    /// Assembles the ordered display list: in_progress → queued → cached done, capped at 5.
    private func buildGroupDisplay(
        ...
        for grp in inProgress { if display.count < 5 { display.append(grp) } }
        for grp in queued     { if display.count < 5 { display.append(grp) } }
