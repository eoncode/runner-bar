...
/// Fixed-capacity ring buffer used to track metric history for sparklines.
struct RingBuffer {
    ...
    init(capacity: Int, fill: Double = 0) {
        ...
        self.values = Array(repeating: fill, count: capacity)
    }
...
final class SystemStatsViewModel: ObservableObject {
    ...
    var memHistory = RingBuffer(capacity: 60)
    var diskHistory = RingBuffer(capacity: 60)
    ...
    private func sample() {
        ...
        DispatchQueue.main.async {
            ...
            self.memHistory.append(mem.total > 0 ? (mem.used / mem.total) * 100 : 0)
            self.diskHistory.append(disk.total > 0 ? (disk.used / disk.total) * 100 : 0)
        }
    }
}
