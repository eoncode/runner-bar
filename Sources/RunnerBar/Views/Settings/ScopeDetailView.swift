...
// MARK: - Sections
/// Extension on `ScopeDetailView` providing section sub-views.
...
/// Extension on `ScopeDetailView` providing failure-hook row sub-views.
extension ScopeDetailView {
        ...
        HStack(spacing: 8) {
            ...
            if isEditingPath {
                ...
                Button(action: { startEditingPath() }) {
                    Text(localRepoPath.isEmpty ? "Tap to set path\u{2026}" : localRepoPath)
                        .font(.system(size: 11, design: .monospaced))
        ...
        Button(action: { showHookSheet = true }) {
            HStack(spacing: 8) {
                ...
                if let cmd = hookCommand, !cmd.isEmpty {
                    ...
                    Text("Tap to set a command\u{2026}")
                        .font(.system(size: 11, design: .monospaced))
                        .foregroundColor(Color.rbTextTertiary)
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
...
// MARK: - Actions
/// Extension on `ScopeDetailView` providing action methods.
extension ScopeDetailView {
...
/// Extension on `ScopeDetailView` providing reusable layout helpers.
extension ScopeDetailView {
