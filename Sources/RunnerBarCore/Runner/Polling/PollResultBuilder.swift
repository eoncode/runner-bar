...
public struct PollResultBuilder {
    ...
    ///
    /// ⚠️ Per-tier cap: `appendUpTo` fills each tier up to `groupDisplayLimit` relative
    /// to the *current* size of `display`, not a global remaining-slots budget. A large
    /// burst of `.loading` groups can therefore consume the full cap before `.queued`
    /// groups get a slot. This mirrors the identical behaviour in `buildJobDisplay` and
    /// is intentional for simplicity. If a global-cap approach is needed (so each tier
    /// shares the remaining headroom), refactor both methods together in a dedicated PR.
