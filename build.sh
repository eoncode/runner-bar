...
# Escape backslash, ampersand and forward-slash in the secret so it can be
# safely embedded as a Swift string literal (no shell-interpolation risk).
ESCAPED_SECRET=$(printf '%s' "$CLIENT_SECRET" | sed 's/\\/\\\\/g; s/&/\\&/g; s/\//\\\//g')
...
enum Secrets {
    ...
    static let clientSecret: String = "${ESCAPED_SECRET}"
}
SWIFT
